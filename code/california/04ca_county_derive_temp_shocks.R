...
rejoin_spike_dat <- function(map1, map2, call, shock_var, glue_string){
  ...
  shocks_container <- 
    map2(map1, map2,
         ~call(ca_co_prism, glue::glue(glue_string), 
               {{ shock_var }}, .y) %>% 
...
ca_prism_tbls_all <- arrow::open_dataset(prism_files)

## 1.2 Seasonal 30-yr average query ----------
tictoc::tic()
ca_szn_study_per_avg <- 
  ca_prism_tbls_all %>% 
  mutate(date_ymd = as.character(date_ymd),
         date = as.Date(date_ymd, "%Y%m%d"),
         year = year(date),
         month = month(date),
         date_coerce = make_date(year = 2023, 
                                 month = month(date), 
                                 day = day(date))) %>% 
  mutate(astro_season = 
           case_when(
             between(date_coerce, as.Date("2023-01-01"), as.Date("2023-03-19")) ~ "WINTER",
             between(date_coerce, as.Date("2023-12-21"), as.Date("2023-12-31")) ~ "WINTER",
             between(date_coerce, as.Date("2023-03-20"), as.Date("2023-06-19")) ~ "SPRING",
             between(date_coerce, as.Date("2023-06-20"), as.Date("2023-09-21")) ~ "SUMMER",
             between(date_coerce, as.Date("2023-09-22"), as.Date("2023-12-20")) ~ "FALL",
             TRUE ~ "WHY")
  ) %>% 
  group_by(GEOID10, astro_season,
           .drop = FALSE) %>% 
  summarise(pptstudy_per_avg = mean(ppt),
...
ca_szn_study_per_avg <- 
  get_heat_indices(ca_szn_study_per_avg,
...
ca_ann_szn_fin <-
  ca_ann_szn %>% 
  left_join(ca_szn_study_per_avg, by = c("GEOID10", "astro_season")) %>% 
  mutate(tmax_anomaly = mean_tmax - tmaxstudy_per_avg,
...
spike_groups <- select(ca_co_prism, contains("_spike")) %>% names()
...
ca_co_szn_yr_derivations <-
  ca_co_szn_yr %>% 
  left_join(ca_szn_study_per_avg, by = c("astro_season", "GEOID10")) %>% 
  mutate(ppt_szn_diff = ppt_szn_avg - pptstudy_per_avg,
         tmean_szn_diff_fahrenheit = tmean_szn_avg_fahrenheit - tmeanstudy_per_avg_fahrenheit,
         tmax_szn_diff_fahrenheit = tmax_szn_avg_fahrenheit - tmaxstudy_per_avg_fahrenheit,
         tmin_szn_diff_fahrenheit = tmin_szn_avg_fahrenheit - tminstudy_per_avg_fahrenheit,
         tdmean_szn_diff_fahrenheit = tdmean_szn_avg_fahrenheit - tdmeanstudy_per_avg_fahrenheit,
         tmean_hi_szn_diff_fahrenheit = tmean_hi_szn_avg_f - tmeanstudy_per_heat_index_f ,
