...
rejoin_spike_dat <- function(map1, map2, call, shock_var, glue_string){
  ...
  shocks_container <- 
    map2(map1, map2,
         ~call(ca_zcta_prism, glue::glue(glue_string), 
               {{ shock_var }}, .y) %>% 
...
ca_szn_study_per_avg <- 
  ...
  mutate(astro_season = 
           case_when(
             between(date_coerce, "2023-01-01", "2023-03-19") ~ "WINTER",
             between(date_coerce, "2023-12-21", "2023-12-31") ~ "WINTER",
             between(date_coerce, "2023-03-20", "2023-06-19") ~ "SPRING",
             between(date_coerce, "2023-06-20", "2023-09-21") ~ "SUMMER",
             between(date_coerce, "2023-09-22", "2023-12-20") ~ "FALL",
             TRUE ~ "WHY")) %>%
...
ca_zcta_prism <- 
  get_heat_indices(ca_zcta_prism,
                   "tmean_heat_index_f", 
                   "tmean_fahrenheit", 
                   "tdmean_fahrenheit", 
                   "fahrenheit")
...
spike_groups <- select(ca_zcta_prism, contains("_spike")) %>% names()
