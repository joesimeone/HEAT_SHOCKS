...
rejoin_spike_dat <- function(map1, map2, call, shock_var, glue_string) {
  shocks_container <-
    map2(
      map1,
      map2,
      ~ call(ore_co_prism, glue::glue(glue_string), {{ shock_var }}, .y) %>%
...
ore_ann_szn_fin <-
  ore_ann_szn %>%
  left_join(ore_szn_study_per_avg, by = c("GEOID10", "astro_season")) %>%
  mutate(
...
spike_groups <- select(ore_co_prism, contains("_spike")) %>% names()
...
ore_co_szn_yr_derivations <-
  ore_co_szn_yr %>%
  left_join(ore_szn_study_per_avg, by = c("astro_season", "GEOID10")) %>%
  mutate(
...
write_csv(ore_co_prism, "fin_data/ore_county10_heatshocks.csv")
