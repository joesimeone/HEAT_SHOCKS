...
phl_prism_ann <- phl_prism_raw %>% 
  ...
  mutate(across(all_of(measures), ~(. * 9/5) + 32, .names = "{.col}_fahrenheit"),
         max_temp_spike = if_else(tmax_fahrenheit > 90,
                                  "Spike", "No Spike"),
         mean_temp_spike = if_else(tmean_fahrenheit > 90,
                                   "Spike", "No Spike")) 
...
## I want variable with year & number of days next to it parentheses for days > 90. Doing that w/ filter and merging back
  ## Better way???
max_year_annotate <- phl_prism_ann %>% 
  filter(max_temp_spike == "Spike") %>% 
  mutate(year_fac = as.factor(year),
         year_max_spike = glue::glue("{year_fac} (days reaching > 90°F = {max_n_spikes})")) %>% 
...
mean_year_annotate <- phl_prism_ann %>% 
  ...
  mutate(year_fac = as.factor(year),
         year_mean_spike = glue::glue("{year_fac} (days over > 90°F = {mean_n_spikes})")) %>% 
...
phl_maxes <- phl_prism_ann %>% 
  group_by(year, max_temp_spike) %>% 
  summarise(max_n_spikes = n()) %>% 
  ungroup() %>% filter(max_temp_spike == "Spike")

phl_for_series <-  phl_maxes %>% 
...
max_labels <- labs(title = "Philadelphia Core Based Statistical Area: Days over 90°F, 2010 - 2020",
...
mean_labels <- labs(title = "Philadelphia Core Based Statistical Area: Days over 90°F, 2010 - 2020",
                    subtitle = "*Estimates taken from University of Oregon's PRISM Daily Spatial Climate Dataset*",
...
spikes_graph <- 
  ...
  geom_vline(xintercept = 90, color = "black", alpha = .95, linetype = "dashed") +
  geom_histogram(bins = bin, binwidth = binwidth, 
                 color = "white", position = "identity") +
  scale_x_continuous(breaks = c(0, 20, 40, 60, 90, 100), 
                     labels = degree_label) +
...
## Maximum temp graphs
max_total_spikes_10to20 <- mean_or_max_graph(tmax_fahrenheit, max_temp_spike, 
                                         max_bins, max_binwidth) + max_labels

max_faceted_spikes <- max_total_spikes_10to20 + 
  facet_wrap(~year_max_spike, scales = "free",
             nrow = 3)


bar_chart_labels <- labs(title = "Philadelphia Core Based Statistical Area: Days over 90°F, 2010 - 2020",
                   subtitle = "*Estimates taken from University of Oregon's PRISM Daily Spatial Climate Dataset*",
                   x = "Year", 
                   y = "Days over 90°F",
                   caption = "1. Data pulled using Google Earth Enginge")
...
temp_spikes_col <- ggplot(phl_maxes, aes(as.factor(year), max_n_spikes)) +
                   geom_col(color =  "white", 
                            fill =   "#07294D", width = .8) + 
                    bar_chart_labels + 
                    joes_dumb_theme

temp_spikes_line <- ggplot(phl_for_series, aes(year_coerce, max_n_spikes)) +
                    geom_line(color = "#07294D", linewidth = .7) + 
                    geom_point() +
                    bar_chart_labels + 
                    joes_dumb_theme
temp_spikes_col
ggsave("phl_max_temp_spikes_90deg_10to20.png", 
       plot = max_total_spikes_10to20, 
       device = "png",
...
ggsave("phl_facet_max_temp_spikes_90deg_10to20.png", 
       plot = max_faceted_spikes, 
       device = "png",
       path = (here("temp_spikes")),
       height = 8,
       width = 10,
...
ggsave("phl_max_temp_spikes_bar_90deg_10to20.png", 
...
ggsave("phl_max_temp_spikes_line_90deg_10to20.png", 
...
#                  color = "white", position = "identity") +
#   scale_x_continuous(breaks = c(0, 20, 40, 60, 90, 100), labels = degree_label) +
#   scale_fill_manual(values = fill_colors) + joes_dumb_theme
# 
